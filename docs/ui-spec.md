...
这份文档是 aIdea 及其外部子项目的 UI 统一规范。独立项目开发时，先阅读并遵守本文档；不要求引用 aIdea 仓库中的组件代码。
...
外部项目可以自行实现组件，但组件行为、视觉和交互应与本文档一致。
...
所有图标从 lucide-react 导入，不使用 emoji 或内联 SVG。

标准按钮变体：

- 主要操作：`default`
- 次要操作：`outline`
- 图标按钮：`ghost` + `size="icon"`
- 删除操作：`destructive`
- 链接：`link`

每个视图最多一个 `default` 主要按钮；若两个操作权重相等，均降级为 `outline`。
...
独立子项目不需要依赖 aIdea 的 `packages` 或源码目录。开发时由 AI 读取本文档，并在自身仓库中按自身技术栈实现等价的组件和视觉规范。
