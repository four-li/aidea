...
当前已实现的安装路径是固定 commit 的源码安装。预编译 `binary` 安装、Apple Silicon/Intel 产物选择、SHA-256 校验和 Python/Node 环境检查尚未由 aIdea 平台实现；在这些能力发布前，官方应用不得在 `aidea.yaml` 中声明 `runtime: binary` 或使用未实现的 `artifacts`、`requirements` 字段。
...
- `runtime` 是已实现或已发布的运行时类型。当前只支持源码安装下的 `node`、`python`、`system` 等说明性值；未来的 `binary` 只有在平台实现预编译包安装后才能使用，不能只靠填写字符串获得开箱即用能力。
...
安装和更新都先在临时 staging 目录获取固定 revision 或预编译包、执行安装/解压步骤，并在平台真正实现后完成健康检查，再替换当前运行版本。失败时保留旧版本和错误日志；更新失败应恢复更新前的运行状态。`install-state.yaml` 记录安装版本、来源、固定 revision 或产物校验信息及定义快照，只用于离线恢复已安装应用，不能替代市场刷新。
...
邮件官方应用固定使用应用 ID `mail-center` 和自己的 `AIDEA_APP_DATA_DIR/app.db`。它是新应用，不迁移、不读取、不兼容旧内置邮件的账户、索引、凭据或数据库；旧内置邮件的删除由 aIdea 后续版本按平台发布契约执行。
