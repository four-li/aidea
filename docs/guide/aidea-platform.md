...
官方应用的运行顺序为：市场展示 -> 校验定义 -> 获取固定源码或预编译包 -> 安装/解压 -> staging 本地健康检查 -> 替换 `source/` -> 启动 -> WebView 展示 -> 更新或卸载。当前正式市场应用使用单个 arm64 `binary` 包的下载、SHA-256 校验和受限解压。固定 revision 的源码安装实现仍保留，但 Python、Node 等系统运行时的环境检查尚未实现，只用于现有开发验证，不得作为新正式应用的安装路径。
