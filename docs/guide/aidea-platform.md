...
日志始终开启。应用管理的“日志”打开统一工作台：官方应用分别查看运行、安装更新和平台事件；内置应用分别查看运行和平台事件；高级设置可查看 aIdea 系统日志并设置全局保留天数与总容量。壳必须保留启动、更新、安装、健康检查、生命周期失败和未处理前端异常的可复制证据。

aIdea 异常退出后，只接管存在有效运行记录、PID 存活、命令参数、工作目录、版本、健康检查地址和独立进程组均与当前安装定义一致，且 `/health` 仍成功的进程。运行记录意外丢失时，若固定端口的监听进程仍与当前安装定义一致且 `/health` 成功，aIdea 会重新纳管。停止操作仅在确认进程组退出后删除运行记录；未退出时保留记录，以便下次启动继续恢复。

若固定端口被占用但无法安全自动接管，应用管理会提供用户确认后的“释放端口”操作：仅结束监听该应用固定端口的进程。对于身份可确认且是独立进程组组长的官方应用，同时结束该进程组。用户在开发目录手动启动的服务不自动接管或停止，但用户明确确认释放端口时可解除端口占用。

用户手动刷新市场时，开搞先通过 Raw 地址读取官方市场仓库唯一的 `market.yaml`，再读取每个已启用应用仓库默认分支的 `aidea.yaml`，不执行 Git clone 或 checkout，也不依赖平台目录 API。自建 GitLab 使用其仓库的 HTTP 或 HTTPS Raw 地址。市场索引读取失败时保留最近一次成功缓存；索引成功后逐个读取应用定义，单个应用失败不阻断其他应用，成功项整体替换 `runtime/market-cache/`，失败项显示为不可安装的占位卡片。新增应用或调整仓库地址只需更新市场仓库的 `market.yaml`，用户刷新市场即可获取；只有市场入口地址或协议变化时才需要发布新版开搞。
