...
邮件管理不再是 aIdea 的内置应用。新的邮件管理固定为官方应用 `mail-center`，必须使用独立应用仓库；预编译安装能力发布后，再按官方应用规范创建正式 `aidea.yaml`。新应用不能依赖旧邮件的内部 IPC、Rust 命令名、前端组件或旧数据库。
...
官方应用的运行顺序为：市场展示 -> 校验定义 -> 获取固定源码或预编译包 -> 安装/解压 -> staging 本地健康检查 -> 替换 `source/` -> 启动 -> WebView 展示 -> 更新或卸载。当前支持固定 revision 的源码安装，以及单个 arm64 `binary` 包的下载、SHA-256 校验和受限解压；Python、Node 等系统运行时的环境检查仍属于待实现能力。
...
- 官方应用的主页面和 `/settings` 页面都会收到 aIdea 追加的 `aidea_theme=light|dark` 参数；应用据此跟随 aIdea 的主题选择，独立运行时再回退到系统主题。App Bridge v1 只服务主页面，`/settings` 不参与握手，只使用首屏参数。
- 只有声明独立 `process` 的应用可显示“随 aIdea 启动”。
- 每个应用都有 aIdea 提供的通用设置详情页；内置应用可额外注册业务设置组件，官方应用固定提供 `/settings` 页面。`reset_command` 是可选的配置重置能力。
...
官方应用必须提供快速的 `GET /health` 和可访问的 `GET /settings`。`/health` 只检查应用服务是否能接收请求，`/settings` 由应用负责业务设置页面；aIdea 不解析业务字段。
