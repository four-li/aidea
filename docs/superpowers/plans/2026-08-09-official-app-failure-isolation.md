...
在 `official_app_installer` 测试模块创建缺少 `definition` 的临时 `install-state.yaml`，断言返回一个 `id == "legacy-app"` 且 `issue.is_some()` 的 manifest：
...
Run: `cargo test official_app_installer::tests::旧安装记录缺少定义快照会显示异常应用`
...
在 `official_app_installer.rs` 为每条无法读取、解析或恢复定义的官方安装记录构造：
...
Run: `cargo test official_app_installer::tests --quiet`
...
**Files:**
- Modify: `shell-frontend/src/builtin-apps/official-market/index.tsx`
- Modify: `shell-frontend/tests/official-market/OfficialMarketPage.test.tsx`

**Interfaces:**
- Consumes现有 `ipc.listOfficialApps()` 和 `ipc.refreshOfficialApps()`。
- 首次进入页面先显示缓存，再自动刷新；失败时保留缓存并更新 `error`。

- [ ] **Step 1: 写失败测试**

mock `listOfficialApps` 返回缓存列表，mock `refreshOfficialApps` 返回新列表。渲染页面后断言两个 IPC 都被调用，最终显示刷新结果。再添加刷新失败用例：缓存列表仍显示，页面显示错误文字。

- [ ] **Step 2: 运行失败测试**

Run: `npm test -- --run tests/official-market/OfficialMarketPage.test.tsx`
...
`refreshMarket` 发生异常时不得清空 `apps` 或 `installed` 状态；只设置 `error`。保留现有手动刷新按钮和 `refreshing` 禁用状态。
...
```bash
cargo test official_app_installer::tests --quiet
cargo test official_market::tests --quiet
npm test --prefix shell-frontend -- --run tests/manifest-loader.test.ts tests/components/ContentArea.test.tsx tests/components/AppManagementPage.test.tsx tests/official-market/OfficialMarketPage.test.tsx
